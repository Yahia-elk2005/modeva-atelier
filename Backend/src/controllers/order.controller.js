...
exports.addOrder = catchAsync(async (req, res, next) => {
    const { items, shippingAddress, shippingCost, total } = req.body;
    ...
    try {
        ...
        const grandTotal = calculatedTotal + finalShipping;
        const finalTotalString = total || `$${grandTotal.toFixed(2)}`;
        ...
        if (process.env.STRIPE_SECRET_KEY) {
            ...
            if (finalShipping > 0) {
                line_items.push({
                    price_data: {
                        currency: 'usd',
                        product_data: { name: 'White-Glove Courier' },
        ...
        try {
            ...
            const customMessage = `Your order has been placed successfully and is now being processed by our Atelier team.<br><br><strong>Shipping Address:</strong> ${shippingAddress || "Giza Governorate"}<br><strong>Total:</strong> ${finalTotalString}`;
        ...
        res.status(201).json({
            success: true,
            message: "Order placed. Redirecting to secure payment...",
            url: sessionUrl,
            data: order[0]
        });
    } catch (error) {
