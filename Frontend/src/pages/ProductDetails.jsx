...
const ProductDetails = () => {
      ...
      try {
        ...
        if (token) {
          ...
          const exists = wishlistItems.some(
            (item) => (item.product?._id || item.product || item._id) === id,
          );
  ...
  if (loading)
    return (
      <div className="text-center py-5 my-5">Loading product details...</div>
    );
  if (!product)
    return <div className="text-center py-5 my-5">Product not found.</div>;
    ...
    <Container className="my-4 my-md-5 py-4 py-md-5">
      ...
      >
        <Modal.Header closeButton className="rounded-0 border-0 bg-light">
          <Modal.Title style={{ fontFamily: "Playfair Display" }}>
            Atelier Size & Biometric Guide
          </Modal.Title>
        </Modal.Header>
        <Modal.Body className="p-4">
          <p className="text-muted small mb-4">
            All measurements are provided in centimeters (cm) and tailored to
            fit standard international haute couture specifications.
          </p>
        ...
        <Modal.Footer className="border-0">
          <Button
            variant="dark"
            className="rounded-0 text-uppercase px-4"
      ...
      <Row className="g-4 g-lg-5 mb-5 pb-4 pb-md-5 border-bottom">
        <Col md={6} lg={5}>
          <div
            className="bg-light p-2 p-md-4"
            style={{ border: "16px solid #F2F2F2" }}
          >
            <ReactImageMagnify
              {...{
                smallImage: {
        ...
        <Col
          md={6}
          lg={7}
          className="d-flex flex-column justify-content-center pt-3 pt-md-0"
        >
          <p
            className="text-muted small mb-2 text-uppercase fw-bold"
            style={{ letterSpacing: "2px", fontSize: "0.75rem" }}
          >
            {product.category || "CATEGORY"}
          </p>
          <h1
            className="display-6 display-md-5 fw-bold mb-3"
            style={{ fontFamily: "Playfair Display", color: "#1A1A1A" }}
          >
            {product.name}
          </h1>
          <div className="d-flex align-items-center gap-2 mb-4">
            <div
              className="bg-teal text-white fw-bold d-flex align-items-center justify-content-center rounded-0"
          ...
          <h3
            className="fw-bold mb-4 fs-4 fs-md-3"
            style={{ color: "#1A1A1A" }}
          >
            ${product.price}
          </h3>
          ...
          >
            {product.description ||
              "Haute couture bespoke garment crafted at Senopati Atelier. Designed with premium fabrics to ensure absolute elegance and durability."}
          </p>
          ...
          <div className="mt-4 pt-3 border-top d-flex flex-column gap-2 text-muted small">
            <span className="d-flex align-items-center gap-2">
              <strong className="text-dark">Availability:</strong> In Stock &
              Ready to Ship
            </span>
            <span className="d-flex align-items-center gap-2">
              <strong className="text-dark">Delivery:</strong> Complimentary
              White-Glove Courier
            </span>
          </div>
        </Col>
      ...
      <Row className="mb-5 border-bottom pb-5">
        <Col lg={12}>
          <h3
            className="mb-4 text-uppercase fs-4"
        ...
        <Col lg={7} className="mb-5 mb-lg-0">
          {product.reviews && product.reviews.length > 0 ? (
            ...
            <div className="p-5 bg-light border text-center text-muted">
              <p
                className="mb-0"
                style={{ fontFamily: "Playfair Display", fontSize: "1.1rem" }}
              >
                No reviews yet for this silhouette. Be the first to share your
                thoughts.
              </p>
            </div>
          )}
        ...
        <Col lg={5}>
          <div className="p-4 border" style={{ backgroundColor: "#F8F9FA" }}>
            <h5 className="mb-4" style={{ fontFamily: "Playfair Display" }}>
              Submit Your Review
            </h5>
      ...
      <div className="mt-5 pt-4">
        <h3
          className="mb-4 mb-md-5 text-center text-uppercase fs-4 fs-md-3"
          style={{ fontFamily: "Playfair Display", letterSpacing: "1px" }}
        >
          You May Also Like
        </h3>
